...
- 在浏览器中粘贴/编辑片段编号、冲突边、带正整数权重的缝合边（权重为任意正整数，无上限）
- 精确求解（0-1 规划，HiGHS）：冲突边两端必异色，缝合边异色时计入权重，目标为切开权重总和最小；任意大小的整数权重都保持精确比较（包括 2⁵³ 以上仅相差 1 的相邻权重）
...
后端将问题建模为 0-1 规划（PuLP + HiGHS，精确求解）：

- `x[i][k] ∈ {0,1}`：片段 i 是否着 mask k；每片恰一色
- 规范化约束：`x[i][k] ≤ Σ_{j<i} x[j][k-1]`（k>0），强制颜色按片段编号升序首次出现
- 冲突边：两端同色的 one-hot 变量两两互斥
- 缝合边：切开变量 `y` 被约束为两端异色的精确指示（双向约束）
- 任意大整数权重精确化：求解器以双精度保存系数，直接放入 ≥2⁵³ 的权重会让相邻
  大整数合并。切开总量按 1000 进制展开为整数数字与进位等式（矩阵系数始终 ≤10⁶，
  远在 2⁵³ 以内），从最高位开始逐位最小化并固定，等价于对切开总量的普通整数最小化；
  Python 端再用任意精度整数独立校验求解器返回的 0/1 方案（校验失败则加严容差重解）
- 求得最优值后固定精确最优面，逐位最小化各片段颜色得到字典序最小方案；
